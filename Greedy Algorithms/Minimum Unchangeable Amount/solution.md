...
For a collection of coins $S$, by $\operatorname{changeable}(S)$ denote the set of all amounts
...
Assume that, for some $1 \le k \le n$, $\operatorname{changeable}(\lbrace c_1, \dotsc, c_k\rbrace)=[m]$
...
$\operatorname{changeable}(\lbrace c_1, \dotsc, c_{k+1}\rbrace)=[m+c_{k+1}]$: to change any amount
$m + 1 < t \le m + c_{k+1}$, take a coin $c_{k+1}$, then it remains to change $1 \le t-c_{k+1} \le m$
using coins $c_1, \dotsc, c_k$
(and this is doable since $\operatorname{changeable}(\lbrace c_1, \dotsc, c_k\rbrace)=[m]$).

This suggests the following algorithm. If $c_1>1$, then return $1$. If $c_1=1$, then
$\operatorname{changeable}(\lbrace c_1 \rbrace)=[1]$. Start adding coins $c_2, \dotsc, c_n$
one by one. Then, you either find a hole or conclude that
$\operatorname{changeable}(\lbrace c_1, \dotsc, c_n\rbrace)=[p]$, in which case the answer is $p+1$.
